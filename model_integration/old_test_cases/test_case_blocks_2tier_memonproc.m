...
chip_width = max(mem_width,2*core_width);
chip_height = max(mem_height,2*core_height);
...
x1 = chip_width - core_width;
y1 = 0;
w1 = core_width;
h1 = core_height;

x2 = 0;
y2 = chip_height - core_height;
w2 = core_width;
h2 = core_height;

x3 = chip_width - core_width;
y3 = core_height;
w3 = core_width;
h3 = core_height;

x4 = (chip_width-mem_width)/2;
y4 = (chip_height-mem_height)/2;
w4 = mem_width;
h4 = mem_height;
...
%            0     mem_height  core_width    core_height     core.power.total;
%            mem_width - core_width     mem_height  core_width    core_height     core.power.total;
%            0     0     mem_width    mem_height     mem.power.total;
%            0     mem_height  core_width    core_height     core.power.total;
%            mem_width - core_width     mem_height  core_width    core_height     core.power.total];
    map = [x0     y0    w0    h0     core.power.total;
           x1     y1    w1    h1     core.power.total;
           x2     y2    w2    h2     core.power.total;
           x3     y3    w3    h3     core.power.total;
           x4     y4    w4    h4     mem.power.total];
    %blk_num is for splitting the power maps of each die
    blk_num = [4 1];    
...
chip_power = 4*core.power.total + 2*mem.power.total;
...
num_layers_in_stack = length(blk_num);
%%%%%%%%%%%%%%%%%%%%%%%geometry information of the chip%%%%%%%%%%%%%%%%%%%
    die.N = num_layers_in_stack;
    die.N = length(blk_num);
    ...
    power_therm_vec = zeros(1,num_layers_in_stack);
    cur_block = 1;
    for pind = 1:length(blk_num)
        last_block = cur_block + blk_num(pind)-1;
        power_therm_vec(pind) = sum(map(cur_block:last_block,5));
        cur_block = last_block + 1;
    end
    
    %power_therm_vec = ones(1,num_layers_in_stack)*chip_power/2;  %power dissipation of each die
