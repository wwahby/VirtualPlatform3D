...
design.Ng_core = 146.542e3;
design.Ach_mm2 = 0.4129/(design.compression_factor^2);
design.gate_pitch = 985e-9*2/design.compression_factor;
design.min_pitch = 160e-9/design.compression_factor;
design.fmax = 1/ (1.3e-9);
design.w_trans = 45e-9/design.compression_factor;
design.rent_exp = rent_exp_mem;
...
Vdd = [ 1 ]; % Vdd used at each scaling node. If Vdd is constant (or stops scaling after a certain node) you can just have a single entry (or only the first few entries until it stops changing)
...
plot.plot_wld_vs_tiers_cf_rca % just run as script