...
power = zeros(num_thicks,num_stacks,num_perms,num_freqs);
power_density = zeros(num_thicks,num_stacks,num_perms,num_freqs);

wire_power = zeros(num_thicks,num_stacks,num_perms,num_freqs);
rep_power = zeros(num_thicks,num_stacks,num_perms,num_freqs);
temp = zeros(num_thicks,num_stacks,num_perms,num_freqs);
thickness = zeros(num_thicks,num_stacks,num_perms,num_freqs);
npads = zeros(num_thicks,num_stacks,num_perms,num_freqs);
cap_density = zeros(num_thicks,num_stacks,num_perms,num_freqs);
Ltsv_m2 = zeros(num_thicks,num_stacks,num_perms,num_freqs);


ild_cell = cell(num_thicks,num_stacks,num_perms,num_freqs);
psn_cell = cell(num_thicks,num_stacks,num_perms,num_freqs);
power_cell = cell(num_thicks,num_stacks,num_perms,num_freqs);
wire_cell = cell(num_thicks,num_stacks,num_perms,num_freqs);
chip_cell = cell(num_thicks,num_stacks,num_perms,num_freqs);
tsv_cell = cell(num_thicks,num_stacks,num_perms,num_freqs);
    ...
    for nind = 1:length(tiers)
        for pind = 1:num_perms
            for freq_ind = 1:num_freqs
                ...
                power(thind,nind,pind,freq_ind) = core.power.total;
                power_density(thind,nind,pind,freq_ind) = core.power.density;

                wire_power(thind,nind,pind,freq_ind) = core.power.wiring;
                rep_power(thind,nind,pind,freq_ind) = core.power.repeater;
                temp(thind,nind,pind,freq_ind) = core.chip.temperature;
                thickness(thind,nind,pind,freq_ind) = core.chip.thickness;
                npads(thind,nind,pind,freq_ind) = core.psn.Npads;


                ild_cell{thind,nind,pind,freq_ind} = core.chip.iidf;
                psn_cell{thind,nind,pind,freq_ind} = core.psn;
                power_cell{thind,nind,pind,freq_ind} = core.power;
                wire_cell{thind,nind,pind,freq_ind} = core.wire;
                chip_cell{thind,nind,pind,freq_ind} = core.chip;
                tsv_cell{thind,nind,pind,freq_ind} = core.tsv;

                Ltsv_m2(thind,nind,pind,freq_ind) = psn_cell{thind,nind,pind,freq_ind}.Ltsv/psn_cell{thind,nind,pind,freq_ind}.l_unit_cell^2;
                cap_density(thind,nind,pind,freq_ind) = psn_cell{thind,nind,pind,freq_ind}.cap_density;
            end
        end
...
% %             power_density(thind,nind,pind,freq_ind) = power_cell{thind,nind,pind,freq_ind}.density;
% %         end
% %     end
% % end
% 
...
% 
%         for nind = 1:length(tiers)
%             npads_plot(nind) = npads(thind,nind,pind,freq_ind);
%         end
%         
...
% 
%         for nind = 1:length(tiers)
%             temp_plot(nind) = temp(thind,nind,pind,freq_ind);
%         end
%         
...
% 
%         for nind = 1:length(tiers)
%             pow_plot(nind) = power(thind,nind,pind,freq_ind);
%         end
%         
%         plot(tiers,pow_plot)
%     end
...
% 
%         for nind = 1:length(tiers)
%             pdens_plot(nind) = power_density(thind,nind,pind,freq_ind)/1e4;
%         end
%         
