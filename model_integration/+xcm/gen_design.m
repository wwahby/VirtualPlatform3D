...
w_trans_min_m = w_trans;
[Plk_per_transistor, Plk_per_transistor_per_um, Ilk_T] = calc_transistor_leakage_at_temp(chip.temperature, To, Vdd, Vth, Ioff_per_um, w_trans_min_m, width_um, 1);
...
d_tsv = tsv.width_m;
h = tsv.height_m;
t_ox = min((1/2*d_tsv)*0.25, 1e-6);
Ctsv = xcm.calc_tsv_capacitance(epsr_ox, d_tsv, t_ox, h);
Ptsv_single = 1/2*a*Ctsv*f*Vdd^2;
num_tsvs = sum(tsv.per_layer);
%num_tsvs = tsv.num;
Ptsv = Ptsv_single*num_tsvs;
