...
Ng_core = 86e6/4;
Ach_mm2_core = 18.5/4;
gate_pitch_core = 465e-9*2/2;
min_pitch_core = 112.5e-9/2;
fmax_core = 3.6e9;
w_trans = 32e-9;
Vdd_core = 1.25;
...
bulk_cu_rho = 17.2e-9;
resistivities = linspace(bulk_cu_rho,5*bulk_cu_rho,41);
...
for nind = 1:length(num_layers)
    for rind = 1:num_resistivities
        ...
        core.wire.use_em_resistant_metal = 1;   % (-) Allow or disallow use of electromigration-resistant metals below a specified minimum pitch
        core.wire.min_non_em_width = 100e-9; % (m) If use_em_resistant_metal is set to 1, Cu resistivity will be replaced with wire.alt_resistivity_em below this pitch
        core.wire.alt_resistivity_em = rho_alt;    %(Ohm*m) Resistivity of alternate EM-resistant metal
...
xlabel('Bulk Metal Resistivity (Ohm m)')
